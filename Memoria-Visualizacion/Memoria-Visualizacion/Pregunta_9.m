...
-(void)setUpInitialText
{
    ...
    pregunta.text = @"RAM estática es mejor conocida como";
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (UITouch *touch in touches)
    {
        ...
        if([startButton containsPoint:location])
        {
            ...
            int value = 1;
            [data setObject:[NSNumber numberWithInt:value] forKey:@"Pregunta_9"];
            [data writeToFile: path atomically:YES];
            
            //cambio de scene (pregunta)
            Pregunta_10 *scene = [[Pregunta_10 alloc] initWithSize:self.size];
            SKTransition *sceneTransition = [SKTransition fadeWithColor:[UIColor darkGrayColor] duration:1];
            [self.view presentScene:scene transition:sceneTransition];
            ...
            NSMutableDictionary *data = [[NSMutableDictionary alloc] initWithContentsOfFile: path];
            [data setObject:[NSNumber numberWithInt:0] forKey:@"Pregunta_9"];
            [data writeToFile: path atomically:YES];
